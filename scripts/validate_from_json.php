...
echo "🚀 Starting validation of emails from JSON file...\n\n";

// First count the total number of emails
echo "📊 Counting total emails in JSON file...\n";
$totalEmails = $dataManager->countEmails();

if ($totalEmails == 0) {
    echo "❌ No emails found in JSON file!\n";
    exit;
}

echo "📧 Found {$totalEmails} emails to validate\n\n";
...
$filenameWithoutExtension = pathinfo((string) $sourceName, PATHINFO_FILENAME);
...
// Save valid emails
if ($validEmailsList !== []) {
    $validFile = sprintf('valid_emails_%s_%s.txt', $filenameWithoutExtension, $timestamp);
...
if ($invalidEmailsList !== []) {
    $invalidFile = sprintf('invalid_emails_%s_%s.txt', $filenameWithoutExtension, $timestamp);
...
$reportFile = sprintf('validation_report_%s_%s.json', $filenameWithoutExtension, $timestamp);
$report = [
    'summary' => $finalStats,
